...
/**
 * Client for Basic Order Processing Worker
 *
 * This client demonstrates how to interact with the basic (non-boxed) worker
 * implementation of the unified order processing contract.
 *
 * Usage:
 *   1. Start Temporal server: temporal server start-dev
 *   2. Start basic worker: cd samples/basic-order-processing && pnpm dev:worker
 *   3. Run this client: cd samples/order-processing-client && pnpm dev:basic
 */
async function run() {
  logger.info("🚀 Starting Order Processing Client (Basic Worker)...");
  ...
  const orders: Order[] = [
    {
      orderId: `ORD-BASIC-${Date.now()}-001`,
      customerId: "CUST-123",
      items: [
        {
          productId: "PROD-001",
    ...
    {
      orderId: `ORD-BASIC-${Date.now()}-002`,
      customerId: "CUST-456",
      items: [
        {
          productId: "PROD-003",
  ...
  logger.info("📦 Processing orders with basic worker implementation...");
  ...
  logger.info("✨ Done!");
  logger.info("");
  logger.info("💡 Note: This client uses the BASIC worker implementation");
  logger.info("   (Promise-based activities with traditional error handling)");
